...
class FindMyController: ObservableObject {
  ...
  func fetchBitsUntilEnd(
    ...
    repeat {
      ...
      let lastTwoBytesModemID = [UInt8(m.modemID >> 8 & 0xFF), UInt8(m.modemID & 0xFF)]
      let firstTwoBytesModemID = [UInt8(m.modemID >> 24 & 0xFF), UInt8(m.modemID >> 16 & 0xFF)]
      let lastTwoBytesMessageID = [UInt8(m.messageID >> 8 & 0xFF), UInt8(m.messageID & 0xFF)]
      let firstTwoBytesMessageID = [UInt8(m.messageID >> 24 & 0xFF), UInt8(m.messageID >> 16 & 0xFF)]
...
//      adv_key += byteArray(from: validKeyCounter) + byteArray(from: m.messageID) + xorResult
      adv_key = static_prefix + lastTwoBytesMessageID + lastTwoBytesModemID
      adv_key += byteArray(from: validKeyCounter) + firstTwoBytesMessageID + firstTwoBytesModemID + xorResult
      validKeyCounter += 1
